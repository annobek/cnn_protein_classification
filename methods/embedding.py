...
labled = pd.read_csv(cleaned_sequences_path)

# Define Kidera factors (without processing of not-aminoacid characters)
kidera_factors = {
...
# Each aminoacid is embedded with Kidera factors
...
# Sequences processing and padding
max_length = 1500 # adjust to perform training with different sequence lengths
kidera_encoded_sequences = process_sequences(sequences, max_length, kidera_factors, default_kidera_vector)

# Save results
np.savez_compressed(kidera_encoded_path, kidera_encoded_sequences)
